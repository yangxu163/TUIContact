...
{
    //-24 去掉id
    return TPersonalCommonCell_Image_Size.height + 2 * TPersonalCommonCell_Margin + (self.showSignature ? 24 : 0) - 24;
}
...
- (void)setupViews
{
    ...
    [_addBtn setImage:[UIImage imageNamed:@"friend_add"] forState:(UIControlStateNormal)];
    [_addBtn setTitle:@"添加成员" forState:(UIControlStateNormal)];
    _addBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    [_addBtn setTitleColor:[UIColor colorWithHex:@"0x000000"] forState:(UIControlStateNormal)];
    [_addBtn addTarget:self action:@selector(addGroupMember:) forControlEvents:(UIControlEventTouchUpInside)];
    _addBtn.hidden = YES;
...
- (void)layoutSubviews
{
    ...
    CGFloat maxLabelWidth = self.contentView.mm_w - CGRectGetMaxX(_avatar.frame) - 30 - TPersonalCommonCell_rightMargin - 30;
    _name.mm_sizeToFitThan(0, _avatar.mm_h/3).mm_top(_avatar.mm_y).mm_left(_avatar.mm_maxX + TPersonalCommonCell_Margin);
    if (CGRectGetMaxX(_name.frame) >= maxLabelWidth) {
...
//    }
    ...
    _signature.mm_bottom(_avatar.mm_b);
    if (CGRectGetMaxX(_signature.frame) >= self.contentView.mm_w) {
        _signature.mm_w = maxLabelWidth;
    }
    
    _genderIcon.mm_sizeToFitThan(_name.font.pointSize * 0.9, _name.font.pointSize * 0.9).mm__centerY(_name.mm_centerY).mm_left(_name.mm_x + _name.mm_w + TPersonalCommonCell_Margin / 2);
    
    _addBtn.mm_height(15).mm_right(10).mm__centerY(_name.mm_centerY).mm_width(TPersonalCommonCell_rightMargin);
}
...
///添加群组成员
- (void)addGroupMember:(UIButton *)btn {
    if (_delegate && [_delegate respondsToSelector:@selector(addGroupMembers:)]) {
        [_delegate addGroupMembers:self];
    }
}
@end

...
- (void)loadData
{
    ...
    [list addObject:({
        ...
        [inlist addObject:({
            ...
            {
                data.value = TUIKitLocalizableString(RemarkNone);
            }
...
#define SDKPlaceTop
    ...
    [list addObject:({
        ...
        [inlist addObject:({
            TUICommonContactTextCellData *data = TUICommonContactTextCellData.new;
            data.key = TUIKitLocalizableString(ProfileSuggest);
            data.showAccessory = YES;
            data.cselector = @selector(onSuggest:);
            data.reuseId = @"TextCell";
            data;
        })];
...
//    })];
    ...
    [list addObject:({
        ...
        [inlist addObject:({
            ...
            data.textColor = TUICoreDynamicColor(@"primary_theme_color", @"03BDAC");
...
- (void)onSuggest:(TUICommonContactTextCell *)cell {
    TUISuggestView *view = [[TUISuggestView alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
///1v1 发起群聊
- (void)addGroupMembers:(TUICommonContactProfileCardCell *)cell {
    TUIContactSelectController *vc = [[TUIContactSelectController alloc] init];
    vc.selectUserId = self.friendProfile.userID;
    vc.title = @"发起群聊";
    [self.navigationController pushViewController:vc animated:YES];
...
//            createGroupInfo.groupType = GroupType_Public;
            createGroupInfo.groupType = GroupType_Work;
